		...
		if (!user.can('bypassall')) {
			...
			if (room) {
				...
				if (room.settings.modchat && !room.auth.atLeast(user, room.settings.modchat)) {
					if (room.settings.modchat === 'autoconfirmed') {
						this.errorReply(
							this.tr`Moderated chat is set. To speak in this room, your account must be autoconfirmed, which means being registered for at least one week and winning at least one rated game on https://play.pokemonshowdown.com/ (any game started through the 'Battle!' button).`
						);
						if (!user.registered) {
			...
			if (targetUser) {
				...
				if (Config.pmmodchat && !Users.globalAuth.atLeast(user, Config.pmmodchat) &&
					...
					throw new Chat.ErrorMessage(this.tr`On this server, you must be of rank ${groupName} or higher to PM users. You can still challenge them. Get verified by joining our discord (https://discord.gg/8zkgWW8PQm) and typing your showdown name in #verify. You don't have to stay in the Discord server afterwards!`);
				}
