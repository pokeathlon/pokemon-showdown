...
export class TeamValidator {
	...
	getValidationSpecies(set: PokemonSet): [Species, Species] {
		...
		if (ruleTable.has('obtainableformes')) {
			...
			} else if (item.id === 'crystalpiece' && ['giratina', 'arceus', 'regigigas'].includes(species.id)) {
				tierSpecies = dex.species.get(species.id + 'primal');
	...
	validateSet(set: PokemonSet, teamHas: AnyObject): string[] | null {
		...
		if (set.teraType) {
			...
			if (!type.exists || ['Crystal', 'Nuclear', 'Cosmic'].includes(type.name) || type.isNonstandard) {
