...
export class Side {
	...
	constructor(name: string, battle: Battle, sideNum: number, team: PokemonSet[]) {
		...
		this.dynamaxUsed = (this.battle.gen !== 8) && !this.battle.ruleTable?.has('candynamax');
	...
	canDynamaxNow(): boolean {
		if (this.battle.gen !== 8 && !this.battle.ruleTable?.has('candynamax')) return false;
	...
	chooseMove(
		...
		if (dynamax && (this.choice.dynamax || !canDynamax)) {
			if (pokemon.volatiles['dynamax']) {
				...
				if (this.battle.gen !== 8 && !this.battle.ruleTable?.has('candynamax')) {
