...
export class RandomInsTeams extends RandomTeams {
	randomInsSets: Partial<RandomTeamsTypes.RandomSet>[] = RandomBattleSets['gen9chaos'];
	levels: AnyObject = {
		"AG": 55,
		"Uber": 60,
		"(Uber)": 60,
	...
	override randomTeam() {
		...
		let pool: Partial<RandomTeamsTypes.RandomSet>[] = this.dex.deepClone(this.randomInsSets);
		...
		while (pokemon.length < this.maxTeamSize) {
			...
			if (TeamValidator.get('gen6insurgenceag').validateSet({...candidate, level: 100} as PokemonSet, {})) continue;
