...
export class RandomInfTeams extends RandomTeams {
	...
	override randomTeam() {
		...
		while (pokemon.length < this.maxTeamSize) {
			...
			const chosenSpecies = this.sampleNoReplace(namePool.filter(id => !this.dex.mod('gen9').species.get(id).exists));

			const candidate = { ...this.sampleNoReplace(pool.filter(set => set.species === chosenSpecies)), evs: { hp: 84, atk: 84, def: 84, spa: 84, spd: 84, spe: 84 } };
