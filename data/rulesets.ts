...
export const Rulesets: import('../sim/dex-formats').FormatDataTable = {
	...
	ifmovelegality: {
		...
		checkCanLearn(move, species, setSources, set) {
			...
			let tutorMoves = (this.format.mod.includes("pokeathlon") || this.format.mod.includes("chaos"))? { ...fusionMoves, ...PoAfusionMoves } : fusionMoves;
		...
		onValidateTeam(team) {
			...
			for (const [i, set] of team.entries()) {
				...
				if (set.fusion && this.dex.species.get(set.fusion).exists) {
					...
					let fusionTypes = this.dex.species.get(set.fusion).types;
